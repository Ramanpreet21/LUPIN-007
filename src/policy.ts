...
const POLICY_RULES: PolicyRule[] = [
  ...
  },
  {
    id: "rule-exfil",
    binaryName: "curl",
    forbiddenFlags: ["-T", "--upload-file", "--post-file", "--post-data", "-d", "--data", "-F", "--form"],
    ...
    if (rule.forbiddenFlags.includes("*") || rule.forbiddenFlags.length === 0) {
      ...
      for (const flag of rule.forbiddenFlags) {
        const i = args.findIndex((arg) =>
          arg === flag ||
          arg.startsWith(`${flag}=`) ||
          (flag.startsWith("-") && flag.length === 2 && arg.startsWith(flag))
        );
        if (i >= 0) {
          trippedIndex = i;
          break;
...
/**
 * Run a command through the active rules and produce the dashboard's AST shape.
 * Each statement in compound commands (;, &&, ||, |, subshells) is scored independently;
 * the worst result across all statements becomes the final risk score and tripped node.
 */
export function simulatePolicyRule(command: string): AstSimulation {
  const statements = extractCompoundStatements(command);
